...
x\sim N(\mu, \sigma^2)\\
\mu\sim N(0, 1000^2)\\
\sigma^{-2} \sim InvGamma(.0005, .0005)\\
$$
...
logPrior <- function(theta){
    ...
    r2 <- MCMCpack::dinvgamma(exp(2*theta[2]), .0005, .0005)
    res <- r1 + r2 + theta[2]
    return(res)
}
...
Next, we sample $M$ values from a proposal distribution $g(\theta)$.  We will use a multivariate normal distribution with mean $\theta^*$, and covariance $sH^{*-1}$. We set $s=2.3$, which is the smallest value that generates a valid proposal.
...
fr1 <- data.frame(draws$draws) %>%
  mutate(method="BD", sigma=exp(2*log.sigma)) %>%
  select(-log.sigma)
```
...
dimnames(fit.mcmc) <- list(draw=1:10000, var=c("mu","sigma"))
fr2 <- data.frame(fit.mcmc) %>% mutate(method="MCMC")
```

Now let's compare the results, after combining samples using both
methods into a single data frame.
