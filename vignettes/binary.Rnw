...
The first substantial step in the algorithm is to find the posterior
mode, as task that often elicits the reaction of ``easier said than
done.''   We agree that large-scale nonlinear optimization can be
difficult when using standard \proglang{R} functions, like
\func{optim}, or many of the optimization packages described in the R
Task View on Optimization.  There are two common problems with these algorithms:

1.  These algorithms may apply a stopping rule that is based on whether the value of the
objective function is making "sufficient progress,'' or if the optimization
    variables continue to change, from iteration to iteration.  These
    stopping rules can be satisfied even when the gradient of the
    objective function is not flat.  For an unconstrained objective
...
2.    Another limitation of most of these algorithms is that they can be
difficult to use when there is a large number of decision
variables. Search methods like Nelder-Mead (the default algorithm for
...
and \func{trust.optim} requires the Hessian as well.  The Hessian is
...
many programming languages, there are none for \proglang{R} (yet) that are
...
The advantage of both analytic and AD derivatives is that they are ``exact.''
...
this approach.  First, the estimate of the derivative is not exact.  If $h$ is too
...
derivatives.  Second, this computation needs to be done for each
...
model, and quadratically for the Hessian. In contrast, the time to compute a gradient using AD methods is
...
hierarchical models because, as we discussed in Section \ref{sec:sparseHessians}, the
sparsity pattern of a hierarchical model is often easily predictable.
Even if the Hessian were derived analytically, it still may be faster to use \class{sparseHessianFD} for
...
\subsection{High-dimensional MVN distribution}
...
probability models.  The log posterior density, ignoring any normalization constants, is
\begin{align}
...
This package includes a simulated dataset for $N$ households and $k$
...
\subsubsection{Functions included in the package}
...
The following code chunk evaluates the log posterior density at a
random vector.
