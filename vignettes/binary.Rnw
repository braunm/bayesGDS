...
sampling from posterior distributions. The main advantages of BD over MCMC are
...
for Bayesian hierarchical models of large datasets.  We refer the
reader to the BD paper for an explanation and justification of the
algorithm.  For convenience, Algorithm 1 in \citet{BraunDamien2015} is
reprinted here, also as Algorithm \ref{alg:GDS}.
...
choices for running the BD algorithm on hierarchical models.  In this note, we explain how one can implement the BD algorithm in a
scalable way, using packages that were written with that algorithm
in mind.  These packages are listed in Table \ref{tab:packages}, and are
...
(\pkg{bayesGDS}) implements only the rejection sampling phase.
...
equivalent to the posterior density, up to a normalizing factor $1/\Ly$.

Under the conditional
independence assumption, the data likelihood can be factored as
...
The sparsity of the Hessian of $\log\Dy$ is critical to the
scalability of the BD algorithm.  Storing the sparse Hessian in a
compressed format not only reduces the memory requirements needed to
store the Hessian, but also the number of operations needed to
multiplying by, inverting, or decomposing it.  Taking advantage of the
sparsity dramatically speeds up the process of finding the posterior
mode, and sampling from an MVN.  See \citet{BraunDamien2015}, Section 4.



\subsection{Nonlinear optimization}\label{sec:optimization}

The BD algorithm involves finding a posterior mode, $\theta^*$, which
often elicits a reaction of ``easier said than
done.''   Finding $\theta^*$ can be a hard problem for large models when using standard
optimization routines, like the \func{optim} function in base
\proglang{R}. There are two common problems with these algorithms:

\paragraph{Premature stopping}  These algorithms may apply a stopping rule that is based on
whether the optimizer is making "sufficient progress.''  Thus, the
optimizer can stop before the gradient of the
objective function is flat.  For an unconstrained objective
...
parameters is large.   Conjugate gradient methods do not store the full Hessian
...
There are two "good" ways to compute a derivative.  The
...
What matters is that functions that return the gradient and
...
\begin{itemize}
 \item The estimate of the derivative is not exact.  If $h$ is too
...
The numerical precision issue is magnified for higher-order
derivatives.
\item  The finite differencing operation needs to be done for each
element of the variable vector.  The time it takes to compute
the gradient grows linearly with the number of units or variables in the
model, and quadratically for the Hessian. In contrast, the time to compute a gradient using AD is
...
Nevertheless, one can use FD to estimate the Hessian if the Hessian is sparse, \emph{and} the
...
the Hessian.  Then one can call the objective function, the gradient, and the Hessian directly
from the \class{sparseHessianFD} object.  The Hessian is returned as a \class{dgCMatrix}
object, which is a sparse, compressed representation that is supported
by the \pkg{trustOptim} and \pkg{sparseMVN} packages.  More details are in
the \pkg{sparseHessianFD} documentation, and in the example that
follows in this vignette.
...
the \class{Cholesky} object (defined in the \pkg{Matrix} package) is in a sparse, compressed format.  Since the size
...
\funcarg{data} and \funcarg{priors}.  These two latter arguments are specific to this
...
methods are unavailable.  An object of
class \class{sparseHessianFD} contains member functions that return
the function, the gradient, and the Hessian, with the Hessian being in
a sparse, compressed format.  The constructor of a \class{sparseHessianFD}
object consists of the objective function and its gradient, any other
objects that are passed to those functions, and the sparsity pattern
of the lower triangle of the Hessian.  The sparsity pattern is defined
...
and then use the \func{Matrix.to.Coord} function from \pkg{sparseHessianFD}.

<<hessStruct>>=
@

Note that only the lower triangular elements should be included.
...
We can also confirm that the estimate of the Hessian is correct.
