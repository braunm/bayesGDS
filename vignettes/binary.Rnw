...
Fortunately, there are better alternatives for running the BD
algorithm on hierarchical models. These packages are listed in Table \ref{tab:packages}, and are
...
The posterior mode $\theta^*$ is the maximum of $\log\Dy$.  If
$\theta$ is unconstrained, then the norm of the gradient of
$\log\mathcal{D}(\theta^*|y)$ is zero.  The \func{trust.optim}
function uses curvature information from the Hessian to iteratively
converge to $\theta^*$.  Unlike other nonlinear optimization routines
for \proglang{R}, \func{trust.optim} takes the Hessian as a sparse
\class{dgCMatrix} object, so it is scalable for large problems.  For the \funcarg{fn}, \funcarg{gr} and \funcarg{hs}
...
Next, we defines functions to sample from a proposal distribution $\Gtheta$, and
compute its density.  The \func{sample.GDS} function will require the
proposal functions to take a variable vector as the first argument, and list of
proposal parameters as the second argument.  That means we need some
wrapper functions.  Since the Hessian is sparse, we can use the
\pkg{sparseMVN} package for an MVN proposal distribution. As discussed in Section \ref{sec:MVN}, the
...
next code chunk, we allocate 10 cores for parallel processing, and set
a random seed for simulating random variables (more on that later).
The \variable{run.par} flag controls whether sampling is done serially
or in parallel.
...
$\Phi(\theta|y)=\dfrac{\Dy}{c_1}\dfrac{c_2}{\Gtheta}$, where
$c_1=\mathcal{D}(\theta^*,y)$ and $c_2=g(\theta^*)$. We sample $M$
times from the proposal distribution, and compute
$\log\Phi(\theta|y)$ for each proposal sample.
...
If $M$ is too small, there may not be enough proposal draws to confirm
...
The \func{foreach}
function runs \func{sample.GDS} in parallel on multiple processing
