...
\newcommand{\Like}{f(y|\theta)}
 \newcommand{\Prior}{\pi(\theta)}
 \newcommand{\Post}{\pi(\theta|y)}
 \newcommand{\LikePrior}{f(y|\theta)\pi(\theta)}
...
knitr::opts_chunk[["set"]](collapse = FALSE, eval=TRUE, echo=TRUE, comment = "#", message=FALSE) #
knitr::read_chunk("binary_chunks.R")
@ 



\citet{BraunDamien2014}, henceforth known as BD, introduce a method of
sampling from posterior distributions.  Although the theory behind the
BD method is general
enough for any bounded posterior density, it works best when the
posterior density is differentiable.  More importantly, the algorithm
is \emph{scalable} for
hierarchical models with conditionally independent heterogeneous
units.  The \pkg{bayesGDS} package provides some tools to
implement the BD method; other packages provide other tools that may
be useful.  This note describes how one might choose to implement the
BD algorithm for a hierarchical model using these tools.  


\section{Prerequisites}

It is assumed that the reader has read \citet{BraunDamien2014}, and is
somewhat familiar with the algorithm.   For convenience, we include
the Algorithm as Algorithm \ref{alg:GDS}.
...
\end{algorithm}
 ...
 In Table\ref{tab:packages}, we list some additional packages that are
 required to run the code in this vignette.  We also explain why they are
useful, and map them to the relevant lines in Algorithm
\ref{alg:GDS}.  The \pkg{sparseHessianFD}, \pkg{sparseMVN}, and
\pkg{trustOptim} packages were developed with the BD algorithm in
mind, so you may want to use them in your own code.
...
The packages in Table \ref{tab:packages} have a common theme:  sparse
matrices.  The importance of sparse matrices in hierarchical models is
...
if $N=\Sexpr{NN}$, $k=\Sexpr{kk}$, and $p=\Sexpr{pp}$, then there are \Sexpr{nv1} total variables, and the Hessian will have the following pattern.
...
The BD algorithm can work for "non-hierarchical" models without sparse
...
of class \class{sparseHessianFD} is constructed with a
...
regions in which the LPD surface is flat.  The \func{trust.optim} function
in the \pkg{trustOptim} package meets those criteria.  To use
\func{trust.optim}, the user must provide R functions that return the LPD
...
\func{binary.hess} function). Fortunately, hierarchical models
...
extract the sparsity pattern using the \func{Matrix.to.Coord} function in
...
The variable \code{post.mode} corresponds to $\theta^*$ in \citet{BraunDamien2014}.
...
in \citet{BraunDamien2014}, \variable{log.c1} is the log posterior density, and
