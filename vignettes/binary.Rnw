...
equal to the negative Hessian at $\theta^*$, times a scaling constant,
is a reasonable choice for many model.  However, when the covariance
matrix is dense, the complexity of sampling from, and
computing the density of, an MVN is quadratic in the number of
variables.  Also, converting a Hessian to a covariance matrix involves
an explicit matrix inversion.  Even if the Hessian is sparse, its
inverse may not be.
